...
class UserHeader extends Component {
	...
	toggleDropdown() {
		const dropdownEles = Array.from(document.getElementsByClassName('closed'));
	...
	render() {
		return (
			<div className='user-header'>
				...
				<div className='header-dropdown closed'>
					<button
						className='closed'
						onClick={() => (
							<Link to={`/users/${this.state.currentUser.id}/profile`} />
						)}>
						Profile
					</button>
					<button className='closed' onClick={() => this.props.logout()}>
						logout
					</button>
				</div>
			</div>
