...
import HeaderModal from '../modals/header_modal';
...
class UserHeader extends Component {
	...
	toggleDropdown() {
		const dropdownEles = Array.from(document.getElementsByClassName('profile-closed')) || Array.from(document.getElementsByClassName('profile-open'));
		const dropdownContainer = document.getElementsByClassName('profile-closed')[0] || document.getElementsByClassName('profile-open')[0];

		if (dropdownContainer.className === 'profile-closed') {
			dropdownEles.forEach((ele) => {
				ele.classList.remove('profile-closed');
				ele.classList.add('profile-open');
			});
		} else {
			dropdownEles.forEach((ele) => {
				ele.classList.remove('profile-open');
				ele.classList.add('profile-closed');
			});
		}
	}
		...
		return (
			<div className='user-header'>
				<svg
					...
					fill='currentColor'
					viewBox='0 0 18 20'
					onClick={!this.state.showModal ? this.handleOpenModal : this.closeModal}>
				...
				<div className='header-dropdown profile-closed'>
					<HeaderModal/>
				</div>
			</div>
		);
