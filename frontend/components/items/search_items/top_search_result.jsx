...
import { playlistPics, findPlaylistIndx } from '../../../util/general_functions/playlist_pictures';
import { addBackPath } from '../../../actions/path_actions';
import { connect } from 'react-redux';

const TopSearchResult = ({ item, history, addBackPath, userId, playlists }) => {
	...
	if (user_id) {
		type = 'PLAYLIST';
		whereTo = 'playlist';
        let indx = findPlaylistIndx(item.id, playlists);
        imgUrl = playlistPics[indx];
	}
	...
	return (
		<section
			className='top-result-background'
			onClick={() => {
				addBackPath();
			...
			<img
				...
				id='best-result-img'
			/>
			<h1 id='best-result-name'>
				{name ? formatName(name, 25) : formatName(title, 25)}
			</h1>
			<div id='result-type'>
